...
#define OE_CORE_H
    ...
    #define OE_CORE_ASSERT(x, ...) { if (!x) {OE_ERROR("Assertion Failed: {0}", __VA_ARGS__); DEBUG_BREAK; }} 
    #define OE_ASSERT(x, ...) { if (!x) {OE_ERROR("Assertion Failed: {0}", __VA_ARGS__); DEBUG_BREAK; }}
#else
